...
class SklearnRandomForest(Interpretation):
    ...
    def treeToRules(treeModel, featuresList):
       ...
       def recurse(node, depth):
           ...
           if tree_.feature[node] != _tree.TREE_UNDEFINED:
               ...
               codelines.append("{}return {}".format(indent, tree_.value[node]))
       recurse(0, 1)
       codelines=re.sub("\[(\s*[1-9]+[0-9]*\.\s+[0-9]*\.)\]", "valid","</br>".join(str(x) for x in codelines))
       codelines=re.sub("\[(\s*0\.\s+[0-9]*\.)\]", "invalid",codelines)
       return codelines
